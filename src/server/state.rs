...
pub async fn initialize_api_state(db_pool: PgPool) -> Result<AppState, ApiError> {
    ...
    let registrar_agg = Arc::new(postgres_es::postgres_cqrs(db_pool.clone(), registrar_queries, registrar_services));

    // -- UpdateLocationsSaga aggregate --
    let update_locations_tracing_query = TracingQuery::<UpdateLocations>::default();
    let update_locations_queries: Vec<Box< dyn Query<UpdateLocationsSaga>>> = vec![Box::new(update_locations_tracing_query)];
    let update_locations_agg = Arc::new(postgres_es::postgres_cqrs(db_pool.clone(), update_locations_queries, update_locations_services));
    ...
    let location_queries: Vec<Box<dyn Query<LocationZone>>> = vec![Box::new(location_zone_tracing_query), Box::new(weather_query)];
    let location_services = registrar::RegistrarServices::HappyPath(registrar::HappyPathServices);
    let location_agg = Arc::new(postgres_es::postgres_cqrs(db_pool.clone(), location_queries, location_services));

    // -- assemble app state --
    Ok(AppState { registrar_agg, update_locations_agg, location_agg, weather_view, db_pool, })
}
...
impl FromRef<AppState> for RegistrarAggregate {
    fn from_ref(app: &AppState) -> Self { app.registrar_agg.clone() }
}

impl FromRef<AppState> for UpdateLocationsSaga {
    fn from_ref(app: &AppState) -> Self { app.update_locations_agg.clone() }
}

impl FromRef<AppState> for LocationAggregate {
    fn from_ref(app: &AppState) -> Self { app.location_agg.clone() }
}

impl FromRef<AppState> for WeatherViewProjection {
    fn from_ref(app: &AppState) -> Self { app.weather_view.clone() }
}

impl FromRef<AppState> for PgPool {
    fn from_ref(app: &AppState) -> Self { app.db_pool.clone() }
}